...
angular.module('bykeBookingApp', ['ngRoute', 'datatables', 'ui.bootstrap'])
  ...
  .controller("ClubListController", function($scope, $http) {
    $scope.clubs = [];
    $http.get('/clubs')
      .success(function(data) {
          $scope.clubs = data;
  ...
  .controller("ClubReviewController", function($scope, $http) {
    $scope.club={};
    $scope.getUsers = function(val) {
      if (val == null) val=''
       return $http.get('/users/search/'+val).then(function(response){
         return response.data;
       });
     };
    ...
    this.user= {};
    this.addReview=function(user) {
      $http.post('/users/', user, function(error, newUser){
        if (error){
          console.log(error)
        }else{
          console.log(newUser)
  ...
  .controller("ClubController", function($scope, $http, $routeParams) {
    $scope.club = {};
    if ($routeParams.id != null)
    {
      $http.get('/club/'+$routeParams.id).success(function(data){
        $scope.club = data;
      });
    }
  })
  ...
  .controller("UserReviewController", function($scope, $http) {
